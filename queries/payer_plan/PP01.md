...
```sql
SELECT
  FLOOR((payer_plan_period_end_date - payer_plan_period_start_date)/365) AS year_int,
  COUNT(1)                                                                         AS num_patients
FROM cdm.payer_plan_period
GROUP BY floor( (payer_plan_period_end_date - payer_plan_period_start_date) /365)
ORDER BY 1;
```
