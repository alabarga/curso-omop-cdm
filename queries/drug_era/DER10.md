...
,      (tt.min_date + ((AVG(CAST(tt.end_date_num AS BIGINT)) OVER ())) * INTERVAL '1 day') AS avg_date
,      ROUND(STDDEV(tt.end_date_num) over (), 0) AS STDEV_days
,      (tt.min_date + (MIN(CASE WHEN tt.order_nr < .25 * tt.population_size THEN 9999 ELSE tt.end_date_num END) over ()) * INTERVAL '1 day') AS percentile_25_date
,      (tt.min_date + (MIN(CASE WHEN tt.order_nr < .50 * tt.population_size THEN 9999 ELSE tt.end_date_num END) over ()) * INTERVAL '1 day') AS median_date
,      (tt.min_date + (MIN(CASE WHEN tt.order_nr < .75 * tt.population_size THEN 9999 ELSE tt.end_date_num END) over ()) * INTERVAL '1 day') AS percentile_75_date
FROM
    (
