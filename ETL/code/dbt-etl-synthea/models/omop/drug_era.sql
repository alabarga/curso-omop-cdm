...
with ingredient_exposures as (
    select
        ...
        case
            ...
            when coalesce(de.days_supply, 0) > 0 then cast(de.drug_exposure_start_date as date) + cast(de.days_supply as integer) * interval '1 day'
            else cast(de.drug_exposure_start_date as date) + interval '1 day'
...
normalized as (
    select
        ...
        exposure_start_date,
        greatest(cast(exposure_start_date as timestamp), cast(exposure_end_date as timestamp)) as exposure_end_date
    ...
    select
        *,
        case
            ...
            when (cast(exposure_start_date as date) - cast(lag(exposure_end_date) over (partition by person_id, ingredient_concept_id order by exposure_start_date, exposure_end_date) as date)) > 30 then 1
            else 0
        end as new_era_flag
    from normalized
),
...
eras as (
    select
        ...
        sum(cast(cast(exposure_end_date as date) - cast(exposure_start_date as date) as integer)) as days_exposed
    from era_assignment
...
select
    ...
    drug_era_end_date,
    drug_exposure_count,
    greatest(cast(cast(drug_era_end_date as date) - cast(drug_era_start_date as date) as integer) - days_exposed, 0) as gap_days
from eras
